...
fn read_class_binding_fields(
    ...
    Ok((0..binding.field_count).fold(Vec::new(), |mut acc, i| {
        let field = match mem.read_ptr(binding.fields.at(i as _)).data_part() {
            Ok(f) => f,
            Err(_) => return acc,
        };
        ...
        let name = match mem.read_utf8_lossy(field.name.address(), 128).data_part() {
            Ok(n) => n,
            Err(_) => return acc,
        };

        let r#type = match mem.read_ptr(field.r#type).data_part() {
            Ok(t) => t,
            Err(_) => return acc,
        };

        let type_name = match mem.read_utf8_lossy(r#type.name.address(), 128).data_part() {
            Ok(tn) => tn.replace(" ", ""),
            Err(_) => return acc,
        };
...
fn read_class_binding_metadata(
    ...
    Ok((0..binding.static_metadata_count).fold(Vec::new(), |mut acc, i| {
        let metadata = match mem.read_ptr(binding.static_metadata.at(i as _)).data_part() {
            Ok(m) => m,
            Err(_) => return acc,
        };
        ...
        let name = match mem.read_utf8_lossy(metadata.name.address(), 128).data_part() {
            Ok(n) => n,
            Err(_) => return acc,
        };

        let network_value = match mem.read_ptr(metadata.network_value).data_part() {
            Ok(nv) => nv,
            Err(_) => return acc,
        };

        let metadata_entry = match name.as_str() {
            ...
            "MNetworkVarNames" => unsafe {
                ...
                let name = match mem.read_utf8_lossy(var_value.name.address(), 128).data_part() {
                    Ok(n) => n,
                    Err(_) => return acc,
                };

                let type_name = match mem
...
fn read_enum_binding_members(
    ...
    Ok((0..binding.enumerator_count).fold(Vec::new(), |mut acc, i| {
        let r#enum = match mem.read_ptr(binding.enumerators.at(i as _)).data_part() {
            Ok(e) => e,
            Err(_) => return acc,
        };

        let name = match mem.read_utf8_lossy(r#enum.name.address(), 128).data_part() {
            Ok(n) => n,
            Err(_) => return acc,
        };
...
fn read_type_scopes(
    ...
    Ok((0..type_scopes.count).fold(Vec::new(), |mut acc, i| {
        let type_scope_ptr = match type_scopes.element(mem, i as _) {
            Ok(ptr) => ptr,
            Err(_) => return acc,
        };

        let type_scope = match mem.read_ptr(type_scope_ptr).data_part() {
            Ok(scope) => scope,
            Err(_) => return acc,
        };
